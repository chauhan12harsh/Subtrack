...
public class AuthController {
    ...
    public AuthDtos.UserView register(@Valid @RequestBody AuthDtos.Register r) {
        if (users.existsByEmailIgnoreCase(r.email()) || users.existsByUsernameIgnoreCase(r.username()))
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Email or username already exists");
        User u = new User(r.username(), r.email(), encoder.encode(r.password()));
        users.save(u);
        return view(u);
    }
    ...
    private AuthDtos.UserView view(User u) {
        return new AuthDtos.UserView(u.getId(), u.getUsername(), u.getEmail(), u.getBalance(), u.getRole().name());
    }
}